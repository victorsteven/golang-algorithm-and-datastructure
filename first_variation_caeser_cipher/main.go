...
func charShift(c byte, step int) int {
	...
	case c >= 'A' && c <= 'Z':
		if step > 0 {
			return (int(c)+step-'A')%26 + 'A'
		}
		return 'Z' - ('Z'-(int(c)+step))%26
	case c >= 'a' && c <= 'z':
		if step > 0 {
			return (int(c)+step-'a')%26 + 'a'
		}
...
func MovingShift(s string, shift int) (ret []string) {
	s1 := ""
	for i, step := 0, shift; i < len(s); i, step = i+1, step+1 {
		s1 += string(charShift(s[i], step))
	}
	ret, unit := make([]string, 5), len(s1)/5
	if len(s1)%5 != 0 {
		unit++
	}
	for left, i := len(s1), 0; left > 0; i++ {
		if left < unit {
			ret[i] = s1[i*unit:]
			left = 0
		} else {
			ret[i] = s1[i*unit : (i+1)*unit]
			left -= unit
		}
	}
	return
