...
//Copy make an identical copy of the set
...
//Subtraction make a set with the elements that are in the first set, but not in the second
func (s *HashSet) Substraction(ss *HashSet) *HashSet {
	new := s.Copy()
	for _, v := range ss.GetValues() {
		new.Remove(v)
	}
